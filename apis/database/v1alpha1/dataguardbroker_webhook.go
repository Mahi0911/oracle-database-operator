...
func (r *DataguardBroker) ValidateUpdate(oldRuntimeObject runtime.Object) error {
	...
	old, ok := oldRuntimeObject.(*DataguardBroker)
	if !ok {
		return nil
	}

	if old.Status.ProtectionMode != "" && !strings.EqualFold(r.Spec.ProtectionMode, old.Status.ProtectionMode) {
